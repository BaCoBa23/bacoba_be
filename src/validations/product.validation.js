...
const validateUpdateProduct = (data) => {
  ...
  if (
    ...
    !Number.isInteger(Number(data.productTypeId))
  ) {
  ...
  if (
    data.initialPrice !== undefined &&
    (Number(data.initialPrice) < 0 || isNaN(Number(data.initialPrice)))
  ) {
  ...
  if (
    data.salePrice !== undefined &&
    (Number(data.salePrice) < 0 || isNaN(Number(data.salePrice)))
  ) {
